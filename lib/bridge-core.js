...
export function validatePayload(payload) {
  ...
  const unsafeRecordIndex = payload.cookies.findIndex(
    (cookie) => !cookie || typeof cookie !== "object" || Array.isArray(cookie),
  );
  if (unsafeRecordIndex >= 0) {
    throw new Error(`The decrypted payload cookie record ${unsafeRecordIndex + 1} is not an object.`);
  }
...
export function clearCookieValues(cookies) {
...
function copyPartitionKey(partitionKey) {
  ...
  let topLevelSite = String(partitionKey.topLevelSite);
  try {
    topLevelSite = new URL(topLevelSite).origin;
  } catch {
    // Validation reports malformed partition sites before import.
  }
  const copied = { topLevelSite };
...
function partitionIdentity(partitionKey) {
  ...
  const normalized = copyPartitionKey(partitionKey);
  const ancestor =
    typeof normalized.hasCrossSiteAncestor === "boolean"
      ? String(normalized.hasCrossSiteAncestor)
      : "unset";
  return `${normalized.topLevelSite}|${ancestor}`;
}
...
function cookieIdentityWithinPartition(cookie) {
  return JSON.stringify([
    stripLeadingDot(String(cookie.domain ?? "")).toLowerCase(),
    Boolean(cookie.hostOnly),
    String(cookie.path ?? ""),
...
function containsCookieIdentityWithinPartition(cookies, expectedCookie) {
  if (!Array.isArray(cookies) || !expectedCookie || typeof expectedCookie !== "object") {
    return false;
  }
  const expectedIdentity = cookieIdentityWithinPartition(expectedCookie);
  return cookies.some(
    (cookie) =>
      cookie &&
      typeof cookie === "object" &&
      !Array.isArray(cookie) &&
      cookieIdentityWithinPartition(cookie) === expectedIdentity,
  );
}

export async function setCookieWithConflictPolicy(
  cookie,
  conflictMode,
  { getCurrentCookies, setCookie } = {},
) {
  if (conflictMode !== "preserve" && conflictMode !== "overwrite") {
    throw new Error("The selected conflict policy is unsupported.");
  }
  if (typeof setCookie !== "function") {
    throw new Error("A cookie writer is required.");
  }
  if (conflictMode === "preserve") {
    if (typeof getCurrentCookies !== "function") {
      throw new Error("A destination cookie recheck is required in preserve mode.");
    }
    const currentCookies = await getCurrentCookies(cookie);
    if (!Array.isArray(currentCookies)) {
      throw new Error("The destination cookie recheck returned an invalid result.");
    }
    // The caller's Chrome query already constrains the exact partition. Treat that
    // browser-normalized scope as authoritative, then match the remaining identity.
    if (containsCookieIdentityWithinPartition(currentCookies, cookie)) {
      return { preserved: true };
    }
  }
  return { preserved: false, result: await setCookie(cookie) };
}
